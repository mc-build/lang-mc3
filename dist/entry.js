...
function evaluateValueWithEnv(code, env) {
    return bindCodeToEnv(`return (${code})`, env)();
}
...
const tokenize = (str, offset = 0) => {
    ...
    return str.split("\n").reduce((p, n, index) => {
        ...
        if (n[n.length - 1] === "{") {
            ...
            if (v)
                p.push(new Token(index + offset, v));
            p.push(new Token(index + offset, "{"));
        }
        else if (n) {
...
class Block {
    ...
    parse(reader) {
        ...
        if (!this.name.startsWith("BLOCK_")) {
            throw new SyntaxError(`Invalid block name '${this.name}' expected block name to start with 'BLOCK_'`);
        }
        return this;
    }
...
const finishEdit = () => {
    ...
    const code = (edit === null || edit === void 0 ? void 0 : edit.join("\n")) || "";
    const inserts = tokenize(code, Math.random());
    const mergableEnv = deepMerge({}, {}, {}, TempEnv);
    tokens.unshift(...inserts.map(token => {
...
const populateCompileTimeParticipants = (participants) => {
    ...
    compileTime.register(B.literal("macro").then(B.argument("name", B.string()).executes((context) => {
    ...
    function recurse() {
        ...
        do {
            ...
            if (tokens[0].line != lastLine) {
                ...
                try {
                    initEdit();
                    compileTime.execute(runCommandAsTemplateLiteral(cmd, getCompleteContext()), "compileTime");
                    finishEdit();
                }
                catch (e) {
                    added = true;
                    lines.push(runCommandAsTemplateLiteral(cmd, getCompleteContext()));
                    cancelEdit();
                }
                finally {
                    command = "";
            ...
            if (tokens[0].value === "{" && !added) {
                command = runCommandAsTemplateLiteral(command, getCompleteContext());
                TempEnv = _last.env;
                try {
                    initEdit();
                    compileTime.execute(runCommandAsTemplateLiteral(command, getCompleteContext()), "compileTime");
                    finishEdit();
                }
                catch (e) {
                    cancelEdit();
        ...
        const last_line = (_a = tokens[0]) === null || _a === void 0 ? void 0 : _a.line;
        const last = tokens.shift();
        if ((last === null || last === void 0 ? void 0 : last.value) != "}") {
            throw new SyntaxError(`expected '}', got '${last === null || last === void 0 ? void 0 : last.value}'`);
        }
        if (command && !added) {
            lines.push(runCommandAsTemplateLiteral(command, getCompleteContext()));
        }
        let id;
        if (config.dedupe) {
...
function default_1(registry) {
    registry.set(".mc", (filepath) => {
        ...
        const intermediary = path.resolve(process.cwd(), ".mcproject", "il", path.relative(SRC_DIR, filepath) + ".il");
        const howManyGroupsAreThere = Object.keys(groups).length;
        const toString = () => Object.entries(groups)
            .map(([key, value]) => `#${key}\n${value.join("\n")}`)
            .join("\n");
        if (config.debug.il) {
        ...
        builder.build({
            ...
            generatedNamespace: path.relative(SRC_DIR, filepath).replace(".mc", "")+"_generated"
        });
    });
    return {
        exported: {
            ...
            registerParticipant(type, name, participant) {
                if (!participants[type]) throw new Error(`unkown participant type '${type}'`)
            ...
            getOutputFilePath(namespace, type, name) {
                return path.resolve(process.cwd(), 'data', namespace, type, ...namespaces, name);
            },
...
function findAndRegisterBuildInParticipants() {
    const api = {
        ...
        dispatch(type, command, source) {
            if (type === "compiler") {
                return compileTime.execute(command, source)
            } else {
                return builder.execute(type, command, source);
            }
        },
        getOutputFilePath(namespace, type, name) {
            return path.resolve(process.cwd(), 'data', namespace, type, ...namespaces, name);
    ...
    log.log(`found ${dir.length} built in providers registering ${participants.compiler.length + participants.generic.length + participants.top.length} handlers`);
}
